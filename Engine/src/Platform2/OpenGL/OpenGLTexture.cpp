#include <Platform2/OpenGL/OpenGLTexture.hpp>
...
namespace Creepy {
    ...
    OpenGLTexture2D2::OpenGLTexture2D2(const std::filesystem::path& filePath) noexcept : m_filePath{filePath} {
    ...
    OpenGLTexture2D2::OpenGLTexture2D2(uint32_t width, uint32_t height) noexcept :
    ...
    OpenGLTexture2D2::~OpenGLTexture2D2() noexcept {
        ENGINE_LOG_WARNING("Delete A Texture: {}", m_rendererID);
        glDeleteTextures(1, &m_rendererID);
    }

    void OpenGLTexture2D2::BindImpl(uint32_t slot) const noexcept {
        glBindTextureUnit(slot, m_rendererID);
    }

    void OpenGLTexture2D2::UnBindImpl() const noexcept {
        glBindTexture(GL_TEXTURE_2D, 0);
    }

    void OpenGLTexture2D2::SetDataImpl(void* data, uint32_t size) noexcept {
