#include <fstream>
#include <Platform2/OpenGL/OpenGLShader.hpp>
...
namespace Creepy{
    ...
    OpenGLShader2::OpenGLShader2(const std::filesystem::path& vertexPath, const std::filesystem::path& fragmentPath) noexcept : m_name{"Default"}, m_vertexFilePath{vertexPath}, m_fragmentFilePath{fragmentPath}{
        this->createProgram();

        ENGINE_LOG_WARNING("Create A Shader: {}", m_rendererID);
    }

    OpenGLShader2::OpenGLShader2(const std::string& name, const std::string& vertexShaderSources, const std::string& fragmentShaderSources) noexcept : m_name{name} {
    ...
    OpenGLShader2::~OpenGLShader2() noexcept {
        ENGINE_LOG_WARNING("Delete A Shader: {}", m_rendererID);
        glDeleteProgram(m_rendererID);
    }

    void OpenGLShader2::BindImpl() const noexcept {
        glUseProgram(m_rendererID);
    }

    void OpenGLShader2::UnBindImpl() const noexcept {
        glUseProgram(0);
    }

    std::vector<char> OpenGLShader2::readFile(const std::filesystem::path& filePath) noexcept {
    ...
    void OpenGLShader2::createProgram() noexcept {
