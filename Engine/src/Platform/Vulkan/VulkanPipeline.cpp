...
namespace Creepy {
    ...
    VulkanPipeline::VulkanPipeline(const VulkanPipelineSpec& pipelineSpec) noexcept {
        ...
        // ViewPort State
        auto&& viewPortState = createViewportState(pipelineSpec.Viewport, pipelineSpec.Scissor);

        // Rasterizer State
        auto&& rasterizerState = createRasterizationState(pipelineSpec.IsWireFrame);

        // MultiSampling State
        auto&& multiSamplesState = createMultiSamplesState();
        ...
        // Vertex Input State
        auto&& vertexInputState = createVertexInputState(pipelineSpec.Attributes);
        ...
        VULKAN_CHECK_ERROR(m_layout = VulkanContext::GetInstance()->GetLogicalDevice().createPipelineLayout(pipelineLayoutInfo));
        ...
        VULKAN_CHECK_ERROR(m_handle = VulkanContext::GetInstance()->GetLogicalDevice().createGraphicsPipeline(nullptr, graphicPipelineInfo).value);
        
    }

    void VulkanPipeline::Destroy(vk::Device logicalDev) noexcept
    {
    ...
    void VulkanPipeline::Bind(vk::CommandBuffer commandBuffer, vk::PipelineBindPoint bindingPoint) noexcept
    {
